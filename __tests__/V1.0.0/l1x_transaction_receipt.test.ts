...
describe("Testing L1X Transaction Receipt", () => {
    ...
    describe('L1XProvider v1.0.0 > Block Details by Block Number', () => {

        let txHash: string = "";

        it("Amount should get transferred successfully of all parameters passed correctly", async () => {
            let _response = await l1xProvider.core.getBlockByNumber({ block_number : "474353" });

            console.log("🚀 ~ file: l1x_transaction_receipt.test.ts:55 ~ expect ~ _response:", JSON.stringify(_response))
        });
    });
